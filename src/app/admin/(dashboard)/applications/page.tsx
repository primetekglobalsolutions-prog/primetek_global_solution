...
const statusOptions = ['all', 'new', 'reviewing', 'shortlisted', 'rejected'] as const;
...
export default function AdminApplicationsPage() {
  ...
  const uniqueJobs = [...new Map(demoJobs.map((j) => [j.id, { id: j.id, title: j.title }])).values()];
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((app) => (
                  <tr key={app.id} className="border-b border-border last:border-0 hover:bg-surface-alt/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-text-secondary">{app.experience_years}y</td>
                    <td className="px-6 py-4">
                      <select
                        value={app.status}
                        onChange={(e) => updateStatus(app.id, e.target.value)}
                        className={`px-2.5 py-1 rounded-full text-xs font-medium border cursor-pointer ${statusColors[app.status]} focus:outline-none`}
                      >
          ...
          >
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <p className="text-sm text-text-secondary">{selectedApp.experience_years} years</p>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-medium border ${statusColors[selectedApp.status]} focus:outline-none w-full`}
                >
                  {statusOptions.filter((s) => s !== 'all').map((s) => (
                    <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>
                  ))}
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-text-muted mb-1">Recruiter Notes</p>
                <textarea
                  rows={3}
                  defaultValue={selectedApp.notes}
                  placeholder="Add notes about this applicant..."
                  className="w-full px-4 py-3 rounded-lg border border-border bg-white text-navy-900 placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary-400 text-sm resize-none"
                />
              </div>
            </div>
          </div>
