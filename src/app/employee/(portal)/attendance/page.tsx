...
import { MapPin, Clock, CheckCircle2, LogIn, LogOut, AlertTriangle, Loader2 } from 'lucide-react';
...
export default function AttendancePage() {
  ...
  const records = useMemo(() => demoAttendance.filter((r) => r.employee_id === empId), []);
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      // Demo fallback — simulate location
      setCoords({ lat: 17.385 + (Math.random() - 0.5) * 0.001, lng: 78.4867 + (Math.random() - 0.5) * 0.001 });
      setGpsStatus('success');
      setCheckedIn(true);
      setCheckInTime(new Date());
    }
